...
-- cleanup_sec009_batch_a1_orphan_recurrence_draft_proposed.sql이 지운 role +
-- manager_centers를 동일한 특성(같은 센터, 같은 계정, 같은 역할명, status='active',
-- is_owner=false)으로 다시 만든다. 원래 행의 정확한 id(uuid)까지 복원하지는 않는다 —
-- 이 데이터는 실제 업무 데이터가 아니라 테스트 픽스처이므로, id가 달라져도 기능적으로
-- 동일하다. account_center_permissions는 예외 오버라이드일 뿐 기본 상태(무권한) 복원에는
-- 필요 없으므로 복원하지 않는다.
--
-- ⚠️ 계정 이메일 규칙을 추측하지 않기 위해, centerA/MANAGER_B의 id를 하드코딩 추측 대신
-- 아래 두 변수에 직접 채워 넣도록 했습니다 — cleanup 실행 전 STEP 1 미리보기 SELECT
-- 결과에서 center_id와 staff_account_id를 그대로 복사해 넣으시면 됩니다.
--
-- ⚠️ 주의: cleanup 실행과 이 롤백 사이에 sec009-batch-a1-rls.test.ts가 다시 실행되어
-- (이번에 고친 beforeAll 자체 정리 로직으로) 스스로 role/manager_centers를 재생성했다면,
-- 이 롤백은 불필요하거나 중복 행을 만들 수 있다 — 아래 미리보기로 이미 존재하는지
-- 먼저 확인할 것.
-- ============================================================
...
do $$
declare
    -- ⚠️ 아래 두 줄을 cleanup 실행 전 STEP 1 미리보기 결과의 실제 값으로 반드시 교체한 뒤 실행하세요.
