-- ============================================================
-- ⚠️ DRAFT / PROPOSED — DO NOT RUN unless fix_class_booking_deadline_override_draft_proposed.sql was applied ⚠️
-- CLASS-001 롤백 — classes.booking_deadline_min을 NOT NULL(default 0)으로 되돌리고
-- (NULL인 행은 전부 0으로 재백필), reserve_class()를 이 수정 이전(PR #32 적용 후 버전,
-- 즉 P1-12 4개 기능만 있고 개별 수업 예약마감 우선순위 로직은 없는 상태)으로 되돌린다.
--
-- ⚠️ 주의: 이 SQL 적용 이후 관리자가 실제로 개별 수업에 booking_deadline_min을 지정한 적이
-- 있다면(0 이상의 실제 값), 롤백 시 그 값이 전부 0으로 재기록되어 "그 수업만 즉시 마감"으로
-- 바뀐다 — 원래 지정했던 값(예: 60분 전 마감)을 잃게 된다. 롤백 전 반드시
-- `select id, title, booking_deadline_min from classes where booking_deadline_min is not null;`
-- 로 실제 지정된 값이 있는지 먼저 확인할 것.
-- ============================================================

BEGIN;

update classes set booking_deadline_min = 0 where booking_deadline_min is null;

alter table classes
    alter column booking_deadline_min set default 0,
    alter column booking_deadline_min set not null;

comment on column classes.booking_deadline_min is
    '마감/자동처리 시간 (분 단위. 수업 시작 기준 몇 분 전까지 허용할지) — 예약 마감';
