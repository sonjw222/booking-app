/*
  Apple 버튼은 Android 네이티브에서만 숨기고 웹/iOS에서는 보여야 하며,
  Google/Kakao/Naver는 그대로 둔다(로그인/회원가입 두 모드가 공유하는 같은 .social-list
  블록 하나뿐이라 한 곳만 확인하면 양쪽 다 커버됨). 소스 구조를 직접 확인해 회귀를 막는다 — 이 프로젝트엔 컴포넌트
...
describe("app/login/page.tsx — Apple 버튼 노출 정책", () => {
  ...
  it("showAppleButton은 Android 네이티브만 제외하는 정책으로 결정된다", () => {
    expect(source).toContain("setShowAppleButton(shouldShowAppleSignInButton())");
  });
  ...
  it("Apple 네이티브 로그인 호출부(signInWithAppleNative)는 그대로 남아있다(제거 금지 확인)", () => {
    ...
    expect(source).toContain('if (provider === "apple" && isAppleNativeSignInSupported())');
  });

  it("일반 웹의 Apple 로그인은 공용 OAuth 경로로 이어진다", () => {
    expect(source).toContain("provider: provider as any");
    expect(source).toContain('provider === "apple" ? "애플"');
  });
});
