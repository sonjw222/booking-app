...
/*
  P1: 계정 설정(app/settings/account/page.tsx) 화면 — 마이페이지에서 진입할 수 있는지,
  그리고 클라이언트 유효성 검사가 동작하는지만 검증한다. 실제로 비밀번호를 성공적으로
  바꾸는 것까지 실행하면 공유 테스트 계정(TEST_USER_A)의 로그인 비밀번호 자체가 바뀌어
  다른 모든 스펙(auth.setup.ts)이 깨지므로, 성공 케이스는 절대 실행하지 않는다.
*/

test.use({ storageState: MEMBER_AUTH_FILE });

test("마이페이지 → 계정 설정 진입, 새 비밀번호 확인 불일치 시 막힌다 (실브라우저)", async ({ page }) => {
...
test("계정 설정 — 6자 미만 비밀번호는 막힌다 (실브라우저)", async ({ page }) => {
