...
|---|---|
| 우선순위 | P1 |
| 현재 상태 | **웹 푸시는 코드 구현 + 배포 완료, 실기기 수신 확인만 남음(자동 검증 불가 — 사용자가 직접 확인 필요).** 카카오 알림톡·SMS·이메일은 사업자 등록이 필요해 범위 밖(P0-1 PG결제와 동일한 종류의 블로커). |
...
남은 건 자동 검증이 불가능한 영역뿐이다:
1. 실제 브라우저에서 알림 권한 허용 → 알림 발생 → 1분 내 실제 기기에 푸시가 뜨는지 눈으로 확인
