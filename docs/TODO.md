...
- A-11 홈 "내 주변 수업 찾기·한 번에 예약·수강권 관리"가 장식 텍스트일 뿐 링크 없음
- A-12 포인트 잔액이 앱 어디에도 안 보임(내역만 있음, 잔액 조회 RPC 별도 연결 필요)
- A-13 예약 이력이 `/my-reservations`·`/mypage/history`·`/mypage/calendar` 3곳에 분산
- A-14 `/mypage/calendar`가 예약 있어도 표시 없는 사실상 빈 화면
- A-16 결제 완료 화면에 방금 만든 주문(`/purchases`)으로 가는 링크 없음
- A-17 알림함에 모두읽음/일괄삭제/날짜그룹 없음, 카드 탭해도 딥링크 안 됨
- A-18 장바구니 수량조절 UI 없음, 쿠폰 적용상태 표시 없음, 미연동 결제수단 선택 가능
- A-19 로그아웃에 확인 없음(바로 실행)
- A-20 구매내역 "잔여>총" 표시 오류, 0원 상품 표시 없음, 환불안내 문구 카드마다 중복
