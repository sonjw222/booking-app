...
**미착수 (다음 후보)**:
- A-2 홈 종목 아이콘 상당수가 빈 결과로 직행(필라테스/발레/요가/수영 등)
- A-6 검색 결과 카드에 이름뿐, 필터·정렬 없음
...
- A-20 구매내역 "잔여>총" 표시 오류(실데이터 확인 필요 — 관리자 수동 조정으로 인한 정상
  상태일 수도 있어 원인 확정 전 섣불리 고치지 않음) · 0원 상품 표시 없음 · 환불안내 문구
  카드마다 중복
- B-4 스태프 권한 화면 — 편집 대상 역할 불명확, 체크박스 17px, "저장됨" 버튼/상태 혼동
- B-6 수업 등록 폼에 수강권 상품 100개+ 인라인 노출(검색/접기 없음)
- B-8 대량 목록 무페이징(`membership-rules`/`admin-assignments`/`notifications`)
