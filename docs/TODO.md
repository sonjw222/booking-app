...
- **상호 변경 정정신고 완료 확인 후 `app/legal/business/page.tsx`의 "상호" 필드를
  "손장욱"에서 "모하빗"으로 교체** — 2026-09-04 기준 세무서에 상호를 "모하빗"으로 정정
  신청했으나 아직 처리 중. 서비스명(브랜드명)은 이미 전면 "모하빗"으로 교체했지만(같은 날
  CHANGELOG 참고), 상호는 사업자등록증 실제 반영 확인 전까지 의도적으로 안 바꿔둠 —
  완료되면 알려주시면 바로 반영
