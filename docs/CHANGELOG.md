...
가져와, 센터 누적 포인트 거래가 300건을 넘으면 최신 거래와 정확한 누적 잔액이 조회에서 통째로
빠지던 버그를 수정했다. `fetchClasses()` 등이 쓰는 `.range()` 페이지 단위 반복 조회로 교체해
전체 거래를 빠짐없이 가져오도록 했다. 7주 전 로컬에 미커밋 상태로 방치돼 있던 별도 문서화
실험(`docs/20_Execution_Plan.md` 등, `archive/local-wip-2026-07`로 백업 후 폐기)이 지적한 70개
이슈를 현재 코드와 하나하나 대조하는 과정에서 발견한, 그 문서에도 적혀있지 않던 별개의 실제
버그다. 신규 회귀 테스트 `points-history-pagination.test.ts` 추가(1,050건 삽입 — 옛 300건
한도와 새 페이지 경계 1,000건을 모두 넘겨 검증, 수정 전 코드로 먼저 실패 재현 후 수정 적용해
PASS 확인).
...
충분히 자세한 설명"을 현재 39,000원 구독 상품 화면에서 명확히 충족하도록 "상품 안내" 박스
내용만 보완. 결제 로직/API/SQL 변경 없음, 텍스트/마크업만 수정.

- **상품명 옆에 "센터 운영자용 · 플랫폼 구독" 배지 추가** — 일반 회원이 센터에서 사는
  수강권·상품 결제와 완전히 다른 축(센터 운영자가 플랫폼에 내는 B2B 월 구독료)임을 심사관이
  한눈에 구분할 수 있게 함. 기존 `.primary-badge`가 쓰는 `--accent`/`--accent-soft` 토큰
  재사용(새 색상 없음, 다크/라이트 둘 다 대응됨).
- **줄글(prose) → 항목별 리스트로 재구성**: 가격/결제 방식/제공 기능/자동 갱신/해지 5개
  항목을 각각 굵은 라벨로 분리해 "한눈에" 확인 가능하게 함(내용 자체는 기존과 동일 —
  새 정보 추가가 아니라 가독성 재구성).
- **이미지 사용 안 함(의도적 결정)**: 이 저장소에 목적에 맞는 "상품 사진" 에셋이 없고
  (`app/icon.png`는 범용 앱 아이콘이라 상품 이미지로 갖다 쓰면 오히려 부자연스러움), 이미
  가격/주기/기능/해지방법을 전부 명시한 텍스트 설명이 있어 토스 가이드의 "이미지 또는
  충분히 자세한 설명" 중 후자로 충족 — 새 이미지 생성/대규모 디자인 변경을 피하기 위한
  의도적 선택.
- **"가격 미정"/"준비 중" 문구 확인**: 코드에 조건부로 남아있지만(0원 플랜이거나
  `billingEnabled`가 꺼졌을 때만 표시), 심사용 센터(39,000원 플랜 + billing_review_
  override=true)에서는 두 조건 다 해당 안 돼 실제 화면에 노출되지 않음을 확인 — 별도
  수정 불필요.

`npm run build`/유닛테스트 288개 통과. 실제 DOM 구조(`.manager-v3-content` 포함)로 정적
하네스를 만들어 라이트/다크 둘 다 스크린샷으로 가독성 확인(배지·리스트 전부 두 테마에서
또렷하게 보임). 카드 등록 버튼/상태/사업자정보/환불정책 등 이 박스 아래 섹션은 전혀
건드리지 않음. 변경 파일: `app/manager/subscription/page.tsx`.
...
지급할 수 있는 기능을 추가함(사용자 요청 — "회원에게 서비스를 주는 상황"). 회원 상세
시트에 "수강권/상품 지급" 버튼 → 상품 선택(가격 자동입력) + 가격/결제방법 수동 조정 +
관리자 메모. 가격 0원이면 결제방법이 "서비스(무상 지급)"로 고정되고, 0원이 아니면
카드/현금/계좌이체 중 선택해 일반 결제처럼 매출에 반영됨.

- `lib/sales.ts`: `grantProductToMember()` 추가 — 상품의 실제 `product_kind`/횟수/만료
  설정을 그대로 읽어 `memberships` 발급 후 `payments`에 `sale_type='new'`(유상) 또는
  `'service'`(무상)로 기록. 결제 insert가 실패하면 방금 만든 membership을 롤백(고아
  membership 방지 — QA 중 실제로 재현된 버그를 고쳄).
- `add_payments_service_sale_type.sql`: `payments_sale_type_check`에 `'service'` 값 추가.
- 매출 화면(`/manager/sales`)의 "매출구분별" 집계에서 "서비스(무상 지급)"는 항상 0원이라
  금액 표시가 무의미해서 건수("N건")로 표시하도록 변경(`RevenueSummary.bySaleTypeCount`
  추가, 사용자 피드백).
- QA로 유상/무상 두 경로 모두 종단 검증: 지급된 수강권이 매출 집계·결제수단별/구분별
  분류·수업 예약(관리자 직접배치)·예약 취소 시 횟수 복구까지 기존 구매 수강권과 완전히
  동일하게 동작함을 실제 DB 조회 + 브라우저 조작으로 확인.
...
매출/정산의 분할결제·미수금)을 다시 깊게 파서 발견한 것 중 실제로 고칠 만한 3건을 수정.
룸/강사 겹침 처리 방식과 미수금 연결 방식은 실제 업무 방식에 따라 갈릴 수 있는 설계
결정이라 사용자에게 먼저 확인받았다(막기 vs 경고만 → 경고만 선택, 미수금은 원래 결제에
직접 수정 vs 새 결제로 연결 → 새 결제 연결 선택).

- **미수금(외상) 회수 연결** (가장 중요) — 결제 등록 시 미수금을 기록하는 기능은 있었지만
  나중에 실제로 받았을 때 어느 미수금을 갚는 건지 연결할 방법이 없어, "이번 달 미수금
  합계"가 회수 여부와 무관하게 계속 부풀려져 있었다. `add_payments_unpaid_link.sql`의
  `collect_unpaid_payment()` RPC로 새 결제(구분: 미수금 결제)를 만들면서 원래 결제와
  연결하고, 원래 결제의 미수금을 회수한 만큼 원자적으로 차감. 결제 상세 시트에 "미수금
  받기" 버튼 추가(`app/manager/sales/page.tsx`). 실제 5,000원 결제·미수금 3,000원 중
  2,000원 회수까지 브라우저로 확인 완료(원래 행 미수금 3,000→1,000원으로 정확히 감소).
- **룸/강사 시간 겹침 경고** — `create_class_safe`/`create_recurring_classes_safe`/
  `set_class_trainers_safe` 등 서버 어디에도 룸·강사 시간 겹침 검증이 없었다. 일부러 겹치게
  운영하는 센터도 있어 막지는 않고, `lib/classes.ts`의 신규 `checkScheduleConflicts()`로
  수업 생성/수정 폼에서 룸·시간·담당 강사가 바뀔 때마다 실시간으로 겹침을 계산해 경고
  배너만 띄운다(저장은 그대로 진행). 실제로 같은 룸·같은 강사로 겹치는 수업을 만들어
  경고 노출 + 저장 정상 진행까지 확인.
- **스케줄 복사 시 수강권 제한·담당 강사 복사 실패가 조용히 묻힘** — `insertCopiedClasses()`
  (lib/classes.ts)가 복사본에 원본의 수강권 제한·담당 강사를 다시 걸어주는 과정에서 실패해도
  `catch`로 조용히 넘어갔다. 실패 건수를 세어(`CopyResult.failedCount`) 복사 완료 토스트에
  같이 안내하도록 수정(기존 "자동 예약조건 등록 실패 개수 안내"와 같은 패턴).

`npm run build`/`npm run test`(262건) 통과.

## 2026-09-06 — 전체 사용 시나리오 2차 UX 감사 후속 수정 (6건)

수강권 외 나머지 상황(회원: 예약/취소, 문의; 매니저: 센터개설, 수업생성, 알림톡)까지 넓혀서
감사한 뒤 발견된 것 중 실제 코드/동작 문제로 확인된 항목만 수정했다. "카운트 불일치"로
의심됐던 알림톡 발송 대상 인원수는 재현 결과 스크롤 안 한 화면만 보고 오판한 것으로 확인돼
수정하지 않았고, 휴대폰 인증 모달을 닫을 수 없게 막은 건 어뷰징 방지 의도 설계로 확인돼
사용자 확인 후 그대로 유지했다.

- **센터 승인 대기 상태 미고지** — `centers.status`가 pending/rejected여도 매니저는 소속만
  active면 수업·수강권·스태프를 다 세팅할 수 있는데 그 사실을 알려주는 곳이 전혀 없었다.
  `lib/manager.ts`의 `ManagedCenter`에 `approvalStatus` 필드 추가, 신규
  `app/components/PendingApprovalBanner.tsx`를 `app/manager/layout.tsx`에 꽂아 모든
  `/manager/*` 화면에 배너로 노출.
- **수업 시간 "정각(0분)" 입력 시 검증 오류 버그** — `AmPmTimeInput`이 빈 문자열일 때 화면엔
  "오전 12시 00분"을 보여주면서도 실제 `form.start`는 계속 빈 문자열로 남아 있어(드롭다운을
  하나도 안 건드리면) "모두 입력해주세요"에 걸렸다. `app/manager/classes/page.tsx`의 `EMPTY`
  상태를 처음부터 유효한 시각("10:00"/"11:00")으로 채워 화면 표시값과 실제 저장값의
  불일치 자체를 없앰(수업 시간 기본값 개선과 동일한 수정으로 해결).
- **마감 후 취소 시 수강권 차감 여부 미고지** — 센터 설정(`deduct_on_late_cancel`)이 켜져
  있으면 마감 후 취소도 막지 않고 환급 없이 처리하는데, `cancel_reservation()` RPC가 이
  사실을 반환값에 안 담아 회원 화면은 항상 "예약이 취소됐어요"로만 안내했다.
  `fix_cancel_reservation_deduct_notice.sql`로 반환 json에 `deducted` 필드만 추가(로직
  변경 없음), `lib/reservations.ts`/`app/reservation/page.tsx`/`app/my-reservations/page.tsx`가
  이 값에 따라 "취소됐지만 마감 이후라 수강권 1회가 차감됐어요"로 분기.
- **매니저 문의함 에러 삼킴** — `lib/inquiries.ts`의 `fetchCenterThreads()`가 에러를 빈
  배열로 삼켜 진짜 오류와 "문의 없음"을 구분 못 했다. 에러를 던지도록 바꾸고
  `app/manager/inquiries/page.tsx`에 에러 배너 추가(이전엔 try/catch 자체가 없어 실패 시
  로딩 스피너가 영원히 안 사라지는 문제도 같이 있었음).
- **알림톡 템플릿-발송 연결 안 됨** — 템플릿 관리 화면에서 만든 템플릿을 실제 발송
  화면(`/manager/alimtalk/send`)에서 불러올 방법이 없어 매번 처음부터 다시 타이핑해야
  했다. 발송 시트에 "템플릿 불러오기" 드롭다운 추가(`lib/alimtalk.ts`의 기존
  `fetchAlimtalkTemplates()` 재사용).

`npm run build`/`npm run test`(262건) 통과. 승인 대기 배너·수업 시간 기본값·알림톡 템플릿
불러오기는 실제 브라우저로 확인 완료. 마감 후 취소 차감 안내는 `fix_cancel_reservation_deduct_notice.sql`
적용 완료(로직 변경 없이 반환값만 추가라 실제 지각취소 재현 없이도 안전).

## 2026-09-06 — 스튜디오 오너/회원 UX 감사 후속 수정 (10건)

수강권 대분류 기능(바로 아래 항목)을 만든 김에, "처음 앱을 쓰는 스튜디오 오너가 수강권을
만들 때"와 "처음 앱을 쓰는 회원이 수강권을 살 때" 두 관점에서 실제로 화면을 조작해보며
찾은 아쉬운 점을 같은 배치에서 바로 고쳤다.

**매니저(오너) 쪽**(`app/manager/membership-rules/page.tsx`):
- 가격/총횟수를 안 채워도 그냥 0원/0회로 생성되던 문제 → 필수 검증 추가.
- 수강권에 "설명" 입력란이 아예 없어 고객 화면의 ⓘ 툴팁이 죽은 기능이었음 → 설명 입력란 추가.
- 수강권 생성만 되고 수정이 안 돼 오타 하나 고치려 해도 삭제 후 재생성해야 했음 → "수정"
  버튼 + 수정 시트 추가(`updateProduct()` 재사용, [TODO P2-0a](./TODO.md) 완료 처리).
- 그룹명이 자유 텍스트라 오탈자로 그룹이 쪼개질 위험 → 기존 그룹명 자동완성(datalist) 추가.
- 생성 시트 안의 "요일반 수강권"(기존 수업에서만 고름)과 생성 후 "예약조건 추가"(자유
  입력)가 서로 다른 기능인데 설명이 없어 헷갈렸음 → 안내 문구 추가.
- 센터를 여러 개 운영할 때 지금 어느 센터를 보고 있는지 헷갈리기 쉬웠음 → "지금 보는 센터"
  라벨 추가.

**회원 쪽**(`app/center/[id]/page.tsx`, `app/checkout/page.tsx`, `app/cart/page.tsx`,
`lib/orders.ts`):
- "담기"/"구매" 버튼 차이가 설명 안 되고 장바구니에 몇 개 담겼는지 확인할 방법이 없었음 →
  안내 문구 + 장바구니 개수 뱃지 추가(`lib/cart.ts`의 기존 미사용 `cartCount()` 활용).
- **가족(다중 프로필) 계정이 구매해도 어느 프로필 앞으로 구매하는지 고를 방법이 없어 무조건
  대표 프로필로 자동 배정되던 문제** → `createOrder()`에 `profileId` 옵션 추가(생략 시 기존
  동작 그대로 하위 호환), 결제 화면·장바구니 화면에 프로필이 2개 이상일 때만 보이는 선택
  UI 추가.
- 결제 화면에 환불정책/이용약관 링크가 전혀 없었음 → 결제 버튼 아래 링크 추가.
- 그룹으로 묶인 수강권이라도 정확히 무슨 요일·시간에 쓸 수 있는지 목록에서 안 보였음 →
  `fetchRulesForProducts()`(기존 N+1 방지 배치 조회 함수)로 예약조건을 미리 불러와 카드에
  요약 표시(비로그인은 RLS로 조용히 생략).
- 수강권이 많아지면 뭘 골라야 할지 판단하기 어려웠음 → 그룹 안에서 가격 오름차순 정렬.

`npm run build`/`npm run test`(262건) 통과. 브라우저로 회원 쪽 전부(장바구니 뱃지, 프로필
선택·전환, 환불 링크, 가격 정렬) 실제 확인. 매니저 쪽은 테스트 계정의 `pass.create` 권한이
감사 도중 사라져(코드와 무관한 계정 상태 변화) 생성 UI 직접 재현은 못 했고 타입체크·코드
리뷰로만 검증함 — 다음에 매니저 권한 있는 계정으로 열어보면 한 번 더 확인 필요.

## 2026-09-06 — 수강권 대분류 표시(group_label)

센터에 수강권 종류가 많아지면 회원이 고르기 어렵다는 피드백(지윤이누나 — "요일고정 수강권 /
스케줄링 가능 수강권"처럼 큰 분류로 묶어서 보여달라는 요청)에 대응. 조사 결과 회차(4/8/12회)
는 이미 상품별로 만들 수 있고 요일·시간 조건도 `membership_schedule_rules`(예약조건)로
이미 표현 가능해, 실제로 빠진 건 "묶어서 보여주는 표시용 라벨" 하나뿐이었다.

`products.group_label`(nullable text, `add_product_group_label.sql`) 추가 — 새 테이블/
관계 없이 센터가 자유 텍스트로 입력(고정 2종으로 강제하지 않음, 사용자 확인 후 결정).
매니저 수강권 생성 시트에 "그룹명(선택)" 입력 필드 추가, 생성된 카드에는 그룹명을 작은
태그로 표시. 고객용 상품선택(결제 모달, `app/center/[id]/page.tsx`)에서는 그룹 없는
상품을 헤더 없이 먼저 보여주고, 그룹 있는 상품만 그룹명 헤더로 묶어서 표시(수강권 섹션에만
적용, 상품(goods) 섹션은 그대로 평면 유지 — 이번 요청 범위 밖).

기존 상품 이름/가격은 매니저 UI에 애초에 "수정" 기능이 없어(생성/삭제만 있음) group_label도
생성 시에만 입력 가능 — 기존 수강권에 그룹을 나중에 붙이려면 당장은 SQL `update` 외에
방법이 없다(`docs/TODO.md`에 기록). `npm run build`/`npm run test`(262건) 통과, 실제
브라우저로 매니저 목록 태그 표시 + 고객 구매 모달 그룹 헤더 혼합 표시(그룹 있음/없음 섞인
경우) 확인 완료.

## 2026-09-06 — 토스페이먼츠 카드사 심사관 전용 계정 PG 결제 노출

토스 PG 계약 심사 안내 메일(계약과정 FAQ) 확인 결과, 카드사 심사(계약 심사 이후 별도
10~14 영업일) 기간 내내 실제 결제창이 동작해야 하고, 방법은 "비회원도 결제창까지 접근
가능" 또는 "심사관 전용 테스트 계정 전달" 둘 중 하나 — 후자를 택했다. 신규
`accounts.pg_checkout_override` 컬럼(`add_pg_checkout_reviewer_override.sql`) — 지정된
계정만 전역 `PG_CHECKOUT_ENABLED` 게이트와 무관하게 온라인 결제(카드/카카오페이/토스페이/
계좌이체)를 볼 수 있음. `accounts` 테이블의 "본인 계정 수정" RLS 정책이 컬럼 단위 제한이
없어(auth_id만 확인) 이 값을 그냥 추가하면 누구나 셀프로 켤 수 있는 구멍이 됐을 것 —
휴대폰 인증 배치의 트리거 패턴을 재사용해 운영자(`is_platform_admin()`)만 바꿀 수 있게
막음(`auth.uid() is null`인 SQL Editor 직접 실행은 항상 허용).

`app/checkout/page.tsx`가 전역 플래그가 꺼져 있을 때만 로그인한 계정의 override 값을
추가로 확인(`lib/authAccount.ts`의 신규 `fetchMyPgCheckoutOverride()`)해 `pgCheckoutEnabled`
상태로 통합 — 일반 고객 노출(전역 플래그)은 전혀 안 바뀌고, 지정된 심사관 계정만 예외적으로
전체 결제수단을 봄. `npm run build` 통과.
...
남기고 제거 — 대표님 자택 주소가 그대로 공개돼 있던 부분을 없앰(상세 정보는
`/legal/business`에서 계속 확인 가능, 비회원도 접근 가능해 법적 표시 의무는 유지).
비로그인 상태에서는 "곧 시작하는 클래스"/"내 수강권으로 예약 가능" 섹션을 숨김(종목
둘러보기·센터 정보·내 주변 센터 검색은 비회원도 그대로 이용 가능, 예약 관련 개인화
목록만 가입 유도). `/reservation`의 "로그인이 필요해요" 화면이 전용 CSS 없이 방치돼
있던 것을 발견 — 앱 전역에서 쓰는 공용 `EmptyState`/`ErrorState` 컴포넌트(`/checkout`
등에서 이미 쓰는 것과 동일)로 교체해 디자인 일관성 확보.
...
자동 QA를 철저히 돌리고, 버튼 디자인도 앱 전체 스타일에 맞췄다.

- **버그 발견**: 장바구니 시나리오를 자동 QA로 실제로 돌려보니(가정이 아니라) `/cart`를
  그냥 열람하는 것만으로는 "로그인이 필요해요"가 뜨지 않았다 — `fetchCart()`는 인증 확인
  없이 바로 `cart_items`를 조회하고, RLS가 비로그인 요청에 조용히 빈 배열만 돌려줘 그냥
  "장바구니가 비어 있어요"로만 보였다. 실제로 이 에러가 나는 지점은 **센터 상세 화면에서
  "담기" 버튼을 눌렀을 때**(`handleAddCart` → `addToCart` → `myProfileId()`)였다 —
  `app/center/[id]/page.tsx`에는 애초에 "로그인 하러 가기" 버튼 자체가 빠져 있었음(신규
  발견, 추가함).
- **디자인**: `.error-toast-action`을 반투명 칩에서 `.primary-btn`과 같은 시각 언어(14px
  라운드, 800 굵기)로 다시 그렸다 — 흰 배경 + danger색 글자로 토스트의 붉은 배경 위에서도
  자연스럽게 어울리도록.
- **신규 E2E** `tests/e2e/auth/post-login-return.spec.ts`(4개 시나리오, 전부 실제 이메일
  로그인 왕복): 결제 화면, 센터 상세 "담기"(장바구니), 예약 화면, 그리고 `next` 없이
  일반적으로 로그인했을 때 여전히 홈으로 가는지(회귀 확인). 소셜 로그인(구글/카카오/
  네이버/애플)은 외부 provider라 자동화 범위 밖 — 다섯 갈래가 전부 같은 홈 착지점에
  모이므로 이메일 경로가 그 착지점 로직을 대표해서 검증한다.
- `npm run build`/유닛테스트 262개 통과.

## 2026-08-31 — 로그인 후 하던 작업 이어가기(결제/장바구니/예약 공통)

바로 앞 배치에서 추가한 "로그인 하러 가기" 버튼을 누르면 로그인 후 항상 홈으로만 돌아가
원래 하던 작업(결제/장바구니 담기/예약)을 처음부터 다시 해야 했던 문제 — `lib/postLoginReturn.ts`
신규로 일반화했다. `/login?next=<원래 경로+쿼리>`로 들어오면 그 값을 세션스토리지에 담아두고,
이메일/구글/애플/카카오/네이버 다섯 로그인 경로가 전부(형태는 제각각이어도) 결국 홈에
도착하도록 이미 통일돼 있는 점을 이용해 `app/page.tsx`의 로그인 확인 시점 한 곳에서만
실제 이동을 처리한다(다섯 갈래 각각을 손댈 필요 없음). 오픈 리다이렉트 방지를 위해
"/"로 시작하는 내부 상대경로만 허용.

적용한 곳: `app/checkout/page.tsx`(결제), `app/cart/page.tsx`(장바구니, 새로 버튼 추가),
`app/reservation/page.tsx`(예약 — 원래 있었지만 아무 데도 안 쓰이던 죽은
`/login?next=/reservation` 링크를 실제로 동작하게 고침). 신규 유닛테스트
`tests/unit/postLoginReturn.test.ts`(오픈 리다이렉트 방지 포함 4개), 실브라우저로 전체
왕복(비로그인 → 결제 시도 → 로그인 → 원래 결제 화면 복귀) 확인. `npm run build`/유닛테스트
258개 통과.

## 2026-08-31 — 결제 화면 "로그인이 필요해요" 에러에 로그인 이동 버튼 추가

사용자 스크린샷 피드백: 비로그인 상태로 결제를 시도하면 "로그인이 필요해요" 에러 토스트만
뜨고 로그인 화면으로 갈 방법이 없었음(직접 로그인 메뉴를 찾아가야 함). `app/checkout/page.tsx`
의 에러 토스트가 이 에러일 때만 아래에 "로그인 하러 가기" 버튼을 추가로 보여주도록 수정
(`/login`으로 이동). 다른 화면들이 공유하는 `.error-toast` 기본 레이아웃은 그대로 두고,
새 조합 클래스(`.error-toast-with-action`)에서만 줄바꿈을 허용해 다른 곳에 영향 없음.
실브라우저로 확인(에러 표시 → 버튼 클릭 → `/login` 이동). `npm run build`/유닛테스트 254개 통과.
...
`center_settings`를 변경하는 통합테스트 파일 10개 전부가 `getOrCreateOwnedTestCenter()`
(2026-08-22 추가된 백스톱 포함)를 거친다는 걸 grep으로 실측 확인 — 우회 경로 없음.
실제 코드(재구현 아님)를 임시 통합테스트로 호출해 라이브 Supabase에서 오염 상태를 인위로
만든 뒤 백스톱이 실제로 되돌리는지도 검증 완료. 근본 원인(누가 왜 오염시키는지)은 여전히
미확정이지만 "오염이 다음 실행까지 전파된다"는 핵심 증상 자체는 구조적으로 막혀 있음이
확인돼 P0 → P2로 하향. 자세한 내용은 `docs/TODO.md` P0-7 참고.

## 2026-08-26 — P0-8 후속: 운영자용 "구독 재개" 추가 + 실제 센터 데이터 복구

QA 도중 운영자가 실제 센터("어텐션 피겨팀")에 구독 취소를 실제로 눌러봤다가 되돌릴
방법이 없어 계속 취소 상태로 남아있던 것을 발견. `admin_reactivate_center_subscription`
RPC 신규 — 카드 등록 여부에 따라 `active`/`pending_billing_setup`으로 복귀,
`/admin/subscriptions`에서 취소된 센터는 "구독 취소" 대신 "구독 재개" 버튼으로 전환.
새 RPC로 어텐션 피겨팀의 구독을 실제로 복구 완료. `npm run build`/유닛테스트 246개
재확인 통과. 자세한 내용은 `docs/TODO.md` P0-8 참고.

## 2026-08-26 — P0-8 후속: 오너 셀프서비스(플랜 변경/구독 취소) + 구독 취소 임시 비활성화

`/manager/subscription`에 오너 본인이 직접 쓸 수 있는 플랜 변경 드롭다운 + 구독 취소
버튼 추가(`center_change_own_subscription_plan`/`center_cancel_own_subscription` RPC
신규, `_is_owner_of_center()` 내부 게이트). 새 플랜의 제한을 현재 사용량이 이미
초과하면 변경 자체를 막음. Playwright 14개 시나리오로 실브라우저 검증(13/14 통과 —
나머지 1개는 공유 fixture 계정이 다른 센터를 이미 소유해서 생긴 테스트 스크립트 오탐,
앱 버그 아님). 검증 직후 사용자가 실제로 취소를 눌러보다 "취소해도 플랜 제한이 그대로라
아무것도 안 멈추는 상태 전환이라 이상하다"는 걸 직접 발견 — 실제 결제 연동 전까지는
구독 취소 버튼을 카드 등록 버튼과 동일한 패턴(`BILLING_ENABLED` 게이트, 회색 처리 +
안내 문구)으로 비활성화. 플랜 변경은 계속 활성 상태(제한 강제는 실제 결제와 무관하게
이미 의미 있음). `npm run build`/유닛테스트 246개 재확인 통과. 자세한 내용은
`docs/TODO.md` P0-8 참고.

## 2026-08-26 — P0-8 후속: 운영자 플랜변경/구독취소 + 오너 전용 게이트 + Playwright 21개 시나리오 검증

`/admin/subscriptions`(조회 전용이었음)에 플랜 변경 드롭다운 + 구독 취소 버튼 추가
(`admin_set_center_subscription_plan`/`admin_cancel_center_subscription` RPC 신규).
"플랫폼 구독" 메뉴를 위임 가능한 권한 키가 아니라 스튜디오 오너로 하드코딩 —
`/manager/subscription` 페이지 자체도 오너 아닌 센터는 제외해 직접 URL 접근도 차단.
Playwright로 메뉴 노출/차단, 폼 입력(무제한 체크박스), 기본 플랜 원자성, 커스텀
확인창, 실제 화면에서 제한 에러 노출, 삭제 보호/해제까지 21개 시나리오 전부 실제
브라우저로 재검증(21/21 통과) — 검증 중 발견한 건 전부 테스트 스크립트 자체 버그였고
실제 앱 버그는 없었음. `npm run build`/유닛테스트 246개 재확인 통과. 자세한 내용은
`docs/TODO.md` P0-8 참고.
...
생성/센터 454개 전부 1:1 backfill/기본 플랜 seed/트리거 정상 확인. Playwright로 매니저
설정 화면의 "플랫폼 구독" 섹션과 운영자 전용 `/admin/subscriptions`(전체 454개 센터 현황)
둘 다 에러 없이 실 데이터로 정상 렌더링되는 것까지 확인. `npm run build`/유닛테스트 246개
재확인 통과. 자세한 내용은 `docs/TODO.md` P0-8 참고.
...
여러 행으로 담아 "행 개수 = 수량"으로 표현하고 +/− 스테퍼 추가(사이즈 있는 상품은 조합이
섞일 수 있어 제외), (2) 쿠폰 적용 상태를 `appliedCoupon` 객체로 명시화하고 체크 아이콘 +
"취소" 버튼 추가, 할인 행에 적용된 쿠폰 코드 표시, (3) "결제 연동 전" 안내를 결제수단 선택
섹션 위로 옮겨 선택 전에 먼저 보이게 함.

## 2026-08-24 — 스태프 권한 화면 명확화 + 수업 등록 폼 수강권 목록 접기(B-4, B-6)

`/manager/staff` 역할별 권한 탭: 편집 중인 역할 이름을 툴바에 표시(항상 "역할을 선택하세요"로
고정돼 있던 버그), 권한 체크박스 17px→22px, 편집 없을 때 "저장됨"을 클릭 가능해 보이는
disabled 버튼 대신 배지로 표시.

`/manager/classes` 수업 등록 폼: "예약 가능 수강권"이 검색 기능은 이미 있었지만(2026-08-06)
신규 등록 시 항상 "전체 선택" 기본값이라 검색 안 하면 97개 칩이 그대로 인라인 노출됐다.
전체 선택 + 미검색 상태면 칩 대신 "전체 N개 목록 보기" 버튼으로 접고, 부분 선택 상태에서는
선택된 항목은 항상 보여주되 미선택 나머지만 20개 초과 시 접는다.

이 접기 때문에 `tests/e2e/admin/class-allowed-products.spec.ts`가 CI에서 3건 실패 —
공유 테스트 센터에 그동안 여러 E2E 파일이 쌓아온 pass 상품이 이미 20개를 훌쩍 넘어 있어
검색 없이 chip을 바로 클릭/카운트하던 기존 테스트가 접힌 목록을 못 찾았다. 검색으로 좁혀서
클릭하도록, "전체 선택" 확인은 접힌 상태에서도 항상 보이는 안내 문구로 바꿔 수정.

## 2026-08-24 — 회원 알림함 날짜 그룹핑 + 페이징(A-17)

`/notifications`(회원)에서 알림이 쌓이면(실측 9,600px+) 구분 없이 쭉 나열되던 것을 "오늘"/
"N월 N일 (요일)" 날짜 헤딩으로 그룹핑하고, 20개씩 "더보기" 페이징을 추가. 카드 탭 시
딥링크 이동과 읽음/안읽음 구분(진입 즉시 자동 읽음 처리)은 리포트가 놓쳤을 뿐 이미
구현돼 있었음 — 확인 후 그대로 둠.

## 2026-08-24 — 예약/취소 후 전체 화면 로딩 깜빡임 제거(A-9)

`/reservation`에서 예약·취소 직후 화면 전체(달력 포함)가 `<Loading/>` 스피너로 통째로
바뀌었다가 다시 그려지던 것을 수정. 원인은 성공 후 호출하는 `load()`가 매번
`setLoading(true)`로 시작해 렌더가 스피너 하나로 완전히 대체되는 구조였던 것 — 데이터
쿼리 로직은 그대로 두고 `load({ silent: true })` 모드를 추가해 이 경로에서는 기존 화면을
유지한 채 데이터만 새로고침하도록 함(진짜 낙관적 갱신은 RPC 반환값을 라이브로 확인해야 해
더 보수적으로 접근). Playwright + MutationObserver로 취소→재예약 왕복 동안 전체 페이지
스피너가 한 번도 안 뜨는 것 확인.

## 2026-08-23 — 매니저 대량 목록 3개 화면 페이징 추가(B-8)

`/manager/membership-rules`(수강권 상품 97개, 이름 검색 입력 신규 추가)·
`/manager/admin-assignments`(기존 필터는 이미 충실했음, 페이징만 추가)·
`/manager/notifications` 3개 화면이 결과를 전부 한 번에 렌더해 스크롤이 10,000px 넘게
늘어지던 것을, `/search`에서 먼저 적용한 것과 동일한 "20개씩 더보기" 패턴으로 완화. 데이터
조회 자체(예: membership-rules가 상품마다 스케줄 규칙을 병렬 조회하는 부분)는 그대로 —
렌더/스크롤 부담만 우선 해결.

## 2026-08-23 — 검색 결과 페이징 추가, 구매내역 0원 상품 표시 수정

검색 결과(실측 131건)가 필터·페이징 없이 전부 렌더되던 것을 20개씩 "더보기"로 점진 노출하도록
수정(A-6). 종목·소개 표시 자체는 이미 구현돼 있었음을 재확인(QA 데이터에 값이 비어있었을
뿐). 구매내역에서 0원 상품이 금액이 아예 안 보이던 것을 "무료"로 명시(A-20). 환불안내
문구·"잔여>총" 표시는 각각 재확인 후 보류(전자는 실제 반복 문구 없음, 후자는 실데이터
확인 필요). A-2(홈 종목 빈 결과)는 정확한 판단에 새 집계 쿼리가 필요해 범위 밖으로 남김.

## 2026-08-23 — 뒤로가기 히스토리 기반으로 전환(C-4), 예약 캘린더 점 표시를 센터 기준으로 변경

`/search`·`/cart`·`/center/[id]`의 뒤로가기(‹)가 무조건 고정 경로("/")로 가던 것을, 앱 안에서
이동해온 경우엔 `router.back()`으로 실제 이전 화면으로 돌아가도록 수정(새 탭/딥링크 진입 시엔
기존처럼 고정 폴백 유지 — `document.referrer` + `history.length`로 판별). 공용
`app/components/BackButton.tsx` 신규.

`/mypage/calendar`(A-14)는 실제로는 이미 점 표시·날짜별 목록·메모·ICS 내보내기가 전부 정상
작동해 감사 리포트가 오래된 상태를 봤던 것으로 결론. 다만 사용자 요청으로 점 색상 기준을
프로필→센터로 변경 — 같은 날 같은 센터 예약이 여러 건이면 점 1개만, 다른 센터면 센터별로
다른 색 점을 표시.

## 2026-08-23 — UI/UX 감사 6차 배치: 운영설정 "준비 중" 토글 실제 버그 발견, 수업매출 소액 표기

운영설정의 "준비 중" 항목 4개 중 3개(시간 입력)는 이미 disabled였는데, 하나("수강권으로
볼 수 없는 수업도 표시" 토글)만 `toggle()` 헬퍼에 disabled 파라미터 자체가 없어서 계속
켜고 끌 수 있고 값도 실제로 저장되고 있었다 — `numInput`과 동일한 패턴으로 통일해 수정.
수업매출 캘린더에서 500원 미만 금액이 "0.0만"으로 반올림돼 0원인지 소액인지 구분 안 되던
것도 그 구간만 정확한 원 단위로 표시하도록 수정.

C-5(저장됨 버튼 상태/액션 혼동)는 코드 확인 결과 로직 자체는 정확해 보류. A-8(동명 수강권
구분)은 실제 재현 확인(이름·만료일이 완전히 같은 수강권 4개)했지만 근본 해결에
`usable_memberships_for_classes()` RPC 수정(새 SQL)이 필요해 이번 배치 범위 밖으로 남김.

## 2026-08-23 — UI/UX 감사 5차 배치: 예약카드 센터명 중복(진짜 원인 발견), 수강권칩 축소, 정원표기 통일, 매출 상품명

예약 화면 수업 카드에서 센터명이 두 번 출력되던 버그의 진짜 원인을 찾았다 — `lib/reservations.ts`의
`place` 필드가 룸 이름이 아니라 센터명을 다시 넣고 있었다(스키마의 `classes.room_id`는
있었지만 안 쓰이고 있었음). 실제 룸 이름을 쓰도록 수정, 룸 미지정 수업은 그 줄이 안 보임.
같은 카드의 수강권 칩이 최대 11개까지 나열되던 것도 "내 수강권으로 예약 가능 · N개" 한 줄
요약으로 축소(수강권 선택은 예약 확인 시트에서 별도로 하므로 기능 손실 없음).

홈 화면의 정원 표기("정원 8명 중 6명 남음")를 센터상세/예약 화면이 이미 쓰던 "예약 N/M"
형식으로 통일. 매출 결제내역 목록에 상품명 추가(CSV 내보내기·상세 모달에만 쓰이던 기존
데이터를 목록에도 노출, 새 조회 없음).

## 2026-08-23 — UI/UX 감사 4차 배치: 로그아웃 확인, 포인트 잔액 표시, 결제완료 링크, 홈 퀵액션

로그아웃에 인앱 확인 모달 추가(A-19, 바로 위 "내 센터 등록하기"와 같은 행 스타일이라 오탭
위험). 포인트 내역 화면 상단에 센터별 합계 "보유 포인트" 카드 상시 표시(A-12, 기존
`fetchAllMyPoints()`/`my_point_balances` RPC 재사용, 새 SQL 불필요). 결제완료 화면의 1순위
버튼을 "마이페이지로"에서 "구매 내역 보기"(`/purchases`)로(A-16). 홈 상단 "내 주변 수업
찾기·한 번에 예약·수강권 관리"가 버튼처럼 보이지만 실제로는 반응 없는 장식 `<span>`이던
것을 각각 `/search`·`/reservation`·`/mypage` 링크로 연결(A-11).

구매내역 "잔여>총" 표시(A-20)는 관리자가 수동으로 수강권 횟수를 추가 지급했을 가능성과
실제 버그를 구분하려면 라이브 데이터 확인이 먼저 필요해 이번엔 보류.

## 2026-08-23 — UI/UX 감사 3차 배치: 매니저 IA 정리(화면 제목 오표기, 네비 중복, 라벨)

`ManagerChrome.tsx`의 `TITLES` 맵에 `/manager/class-revenue`·`/manager/leads`가 없어 기본값
"관리자"로 표시되던 버그 수정. `class-revenue`가 레이아웃이 이미 제공하는 `<ManagerNav/>`를
자체적으로 한 번 더 렌더링해 하단 네비가 2개 겹쳐 보이던 것도 확인·수정(레거시 마이그레이션
잔여물). 매니저 대시보드 "센터 관리" 메뉴에 없던 "회원 관리" 항목 추가, "수강권 관리"→
"수강권·예약조건 설정"으로 라벨 명확화("상품 관리"와 헷갈리던 문제), "복사"→"일정 복사",
"예약자 동기화"에 설명 툴팁 추가.
...
**흐름차단 6건**: 수강권 구매 시트에 항상 보이는 닫기(X) 버튼 추가(상품 100개+로 맨 아래 닫기
버튼까지 스크롤해야 갇히던 문제, UX A-3), 매니저 계정의 "관리자 모드로 전환" 진입점을 마이페이지
최하단에서 프로필 바로 아래로 이동 + 강조(UX B-1), "내 예약" 카드에 취소 버튼 추가(UX A-4) 겸
예약 취소 확인을 브라우저 기본 confirm()에서 인앱 모달로 통일(UX A-10), 매니저 출석 체크 버튼
5개 26px→44px + 파괴적 동작 시각 구분(UX B-3), 수업 행의 "예약 N/M" 21px 텍스트 링크를 44px
독립 버튼으로 분리(UX B-2), 주문취소 확인창 버튼 라벨이 둘 다 "취소"였던 것을 "닫기"/"주문
취소"로 구분(UX A-5).

**센터 상세 페이지(P0-2/A-1)**: CSS margin collapse로 뒤로가기 버튼과 센터 로고가 겹치던 버그
수정, 센터명 가운데정렬+글자단위 줄바꿈을 좌측정렬+어절단위로 수정, 주소·소개가 없으면
700px+ 완전 백지였던 "센터 정보" 탭에 빈 상태 안내 + 대체 동선 추가.

**E2E 후속 수정 2건**: (1) 예약 취소 confirm() → 인앱 모달 전환으로 네이티브 `dialog` 이벤트를
기다리던 E2E 테스트 5개가 깨져 있던 것을 인앱 모달 버튼 클릭으로 수정(관리자 쪽 별도
confirm()들은 안 건드림). (2) E2E job이 정상 실행인데도 `timeout-minutes: 20`에 걸려 두 번
연속 cancelled됨을 확인 — Integration job이 같은 이유로 20→35로 상향됐던 전례(P2-27)와
동일하게 E2E도 20→30으로 상향.

## 2026-08-23 — 다크모드 흰-바탕-흰-글씨 버그 전면 수정 + 라이트모드 --text-dim 대비 미달 수정 (Opus 5 UI 감사 P0)

별도 세션(Opus 5)의 UI 디자인 감사에서 다크모드 핵심 화면(`/manager/sales`의 매출/순이익/미수금
숫자 등)이 대비 1.09:1로 사실상 안 보인다는 P0 발견 사항을 반영. 원인은 `background:var(--ink)`
(다크모드에서 거의 흰색으로 뒤집힘)와 고정 흰색인 `color:var(--text-inverse)`를 짝지은 규칙이
`app/globals.css`에 16곳 남아있던 것 — 이미 올바른 패턴(`color:var(--bg)`, `--bg`는 `--ink`의
반대 극단이라 테마와 무관하게 항상 대비 유지)이 `.fab-btn` 1곳에만 적용돼 있었는데 나머지
15곳(App 버튼, 필터칩, 매니저 크롬 링크, 센터 칩, 상품/진도 마크, 문의 버튼, 마이페이지
아바타·멤버십 CTA, 관리자 primary 버튼, 매출 요약 카드 등)에 동일하게 확장 적용했다.
로그인 히어로(`.auth-scene`/`.auth-page-v2`)는 성격이 달라(하드코딩 `rgba(255,255,255,..)`
장식이 많은, 원래부터 테마 무관 고정 다크 배너) `--ink` 대신 `.admin-chrome`과 동일한 이유로
이미 검증된 `--grad1`(라이트/다크 모두 거의 검정 고정)로 교체.

같은 감사에서 라이트모드 `--text-dim: #747479`가 `--bg` 위에서 4.49:1로 AA(4.5:1) 기준을
0.01 차이로 미달한다는 것도 확인돼 `#6B6B70`으로 조정(감사 보고서 추정 앱 전체 569건 영향).

회귀 방지용 규칙 2개를 `tests/unit/designSystem.contract.test.ts`에 추가 — (1)
`background:var(--ink)` + `color:var(--text-inverse)` 조합이 다시 생기지 않는지, (2)
`--text-dim`이 옛 값으로 되돌아가지 않는지. Playwright로 `/manager/sales`(다크)와 로그인
화면(라이트/다크)을 재검증, 회귀 없음 확인. `npm run build` + 단위테스트(246개) 통과.
...
교체. 카카오는 실제 말풍선 로고, 애플은 실제 사과 로고 SVG로 새로 그렸다(기존엔 애플 아이콘이
아예 비어 있었음) — 애플 로고는 `getBBox()`로 측정한 path 실제 bbox에 맞춰 viewBox를 다시 잘라
원 중앙에 오도록 보정. 버튼 라벨 텍스트는 화면에는 안 보이고 `.sr-only`로 스크린리더용으로만
남겨 접근성을 유지했다.

## 2026-08-22 — 결제수단·길찾기 이모지를 아이콘/벤더 색점으로 재설계

`cart/page.tsx`/`checkout/page.tsx`의 결제수단(💳🟡🔵🏦🤝)과 `center/[id]/page.tsx`의
길찾기 앱 목록(🟡🟢🔵🗺️)이 이모지였던 걸 교체. 카카오페이/토스페이, 카카오맵/네이버지도/티맵처럼
로고 없이 색으로만 구분해야 하는 경우 `--vendor-*` 색 점(`.vendor-dot`)으로, 나머지는 새
outline 아이콘(`card`/`bank`/`handshake`)으로 바꿨다. `--vendor-toss`/`--vendor-tmap` 토큰
신설. 구글 지도는 로고가 다색이라 일반 지도핀 아이콘으로.

## 2026-08-22 — P2-11 후속: 사업자등록번호 표기 정규화 중복 방지

`centers.business_number` unique 인덱스가 원문 그대로에 걸려 있어 "123-45-67890"과
"1234567890"처럼 표기만 다르고 숫자는 같은 번호는 중복 등록을 막지 못했다. 정규화(숫자만
비교) 기준 기존 중복 없음을 읽기 전용 진단 쿼리로 먼저 확인한 뒤,
`regexp_replace(business_number, '\D', '', 'g')` 기준 unique 인덱스로 교체(라이브 적용
확인됨). `register_center_for_account_safe()`의 예외 처리가 범용이라 RPC/클라이언트 코드는
변경하지 않았다.

## 2026-08-22 — 캘린더 4벌 선택 상태 통일 + 매니저 "수업" 헤더 버튼 사라짐 버그 수정

Playwright로 캘린더 4곳(예약/`mypage/calendar`/매니저 수업 "복사" 시트/`class-revenue` 날짜
선택기)에서 실제로 날짜를 선택해보고 스크린샷으로 비교 — 선택 표시가 4가지로 제각각이었다.
예약·매니저 수업 캘린더가 이미 쓰던 brand-soft 원형(연한 하늘색 배경 + 진한 하늘색 글자)을
기준으로 `.mypage-cal-cell.sel`/`.copy-cal-cell.on`/`.app-date-grid button.on`을 맞췄다. 이
토큰은 다크 모드에서도 값이 고정이라 이전 `var(--ink)` 기반 조합의 다크 모드 대비 문제도
함께 없어졌다.

작업 중 매니저 "수업" 화면의 "복사"/"휴무일" 헤더 버튼이 ManagerChrome 공용 헤더 마이그레이션
이후 완전히 안 보이고 클릭도 안 되던 걸 발견해 별도로 수정(`docs/TODO.md` P2-DS-1 참고).

## 2026-08-22 — 다크 모드 실사용 검증(Playwright) + 근본 버그 2건 수정 + 테마 설정 메뉴 복원

Playwright로 실제 브라우저를 띄워 회원·매니저·운영자 화면 전 구간에서 다크 모드를 검증(콘솔
에러 0건까지 확인). 진짜 버그 2건 발견·수정:
- `app/settings/theme/page.tsx`에서만 테마를 적용하고 다른 화면엔 재적용 로직이 없어 페이지
  이동(이 앱은 `<Link>` 대신 `<a href>`라 풀 페이지 이동) 즉시 라이트로 되돌아가던 근본 버그 —
  `app/layout.tsx`에 하이드레이션 전 동기 실행 인라인 스크립트로 고침(`suppressHydrationWarning`
  로 의도된 서버/클라이언트 불일치 경고도 함께 억제).
- `background: var(--ink)` + 고정 `color: var(--text-inverse)` 조합이 다크 모드에서 `--ink`가
  거의 흰색으로 뒤집히며 흰 바탕에 흰 글자가 되던 버그 2곳 수정: 운영자 화면 헤더(`.admin-chrome`,
  배경을 항상-어두운 `--grad1`로), 매니저 수업 화면 "+ 수업 등록" 버튼(글자색을 테마에 따라
  자동 반전되는 `--bg`로). 같은 조합이 앱 전역 20곳 이상 더 있어 후속 과제로 `docs/TODO.md`에
  기록.
- 위 수정 확인 후 `app/mypage/page.tsx`의 "테마 설정" 메뉴 진입 링크를 다시 노출.
