-- ============================================================
-- 수강권-수업 자격판정 공용 함수 + show_all_classes 필터 (P1-9 "준비 중" 항목 4)
--
-- 배경: reserve_class()/reserve_with_membership() 둘 다 "이 수강권으로 이 수업을 예약할 수
-- 있는가"(pass_selection_mode/class_allowed_products/membership_schedule_rules 판정)를
-- 각자 인라인으로 중복 구현하고 있었다. 이전에 auto_book_membership()과 reserve_class()가
-- 이 로직을 각자 조금씩 다르게 구현해 드리프트가 난 적이 있어(docs/TODO.md 참고), 이번엔
-- 그 로직을 딱 한 곳(is_membership_eligible_for_class)에만 두고 예약 RPC 두 개가 그걸
-- 그대로 호출하도록 바꾼다 — 예약 동작 자체는 바뀌지 않는다(로직을 뽑아낸 것뿐).
--
-- center_settings.show_all_classes(기본 true)가 false인 센터에서는, 회원 앱 캘린더가
-- "이 회원의 어떤 수강권으로도 예약할 수 없는 수업"을 걸러서 안 보여준다. true(기본값)인
-- 센터는 지금처럼 전부 다 보인다 — 이 배치를 적용해도 기존 센터는 전혀 동작이 안 바뀐다.
--
-- 여러 번 실행해도 안전(create or replace function).
-- ============================================================

-- ------------------------------------------------------------
-- [1] 공용 자격판정 함수 — "이 수강권으로 이 수업을 예약할 수 있는가"
--     (remaining_count/expires_at/status 같은 수강권 자체의 유효성은 호출부가 각자 확인 —
--     여기서는 "그 수업에 이 상품이 허용되는가"만 판정)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- [2] reserve_class() — 인라인 판정 블록을 is_membership_eligible_for_class() 호출로 교체.
--     나머지 로직은 라이브 정의와 완전히 동일(pg_get_functiondef로 확인 완료).
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- [3] reserve_with_membership() — 동일하게 판정 블록만 교체.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- [4] 회원 캘린더용 배치 필터 — show_all_classes=false인 센터에서 어떤 수강권으로도
--     예약 불가능한 수업 id를 걸러낸다. p_class_ids 중 "이 회원 소유 수강권 중 하나라도
--     예약 가능한" 것만 반환.
-- ------------------------------------------------------------
create or replace function filter_eligible_class_ids(p_membership_ids uuid[], p_class_ids uuid[])
returns uuid[]
language sql
stable
security definer
...
as $$
    ...
    where c.id = any(p_class_ids)
      and exists (
          select 1 from unnest(p_membership_ids) as mid
          where is_membership_eligible_for_class(mid, c.id)
      );
$$;

revoke all on function filter_eligible_class_ids(uuid[], uuid[]) from public;
revoke all on function filter_eligible_class_ids(uuid[], uuid[]) from anon;
grant execute on function filter_eligible_class_ids(uuid[], uuid[]) to authenticated;

-- ============================================================
-- 확인
-- ============================================================
select proname from pg_proc where proname in (
    'is_membership_eligible_for_class', 'filter_eligible_class_ids',
    'reserve_class', 'reserve_with_membership'
) order by proname;
