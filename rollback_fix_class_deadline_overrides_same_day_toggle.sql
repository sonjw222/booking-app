-- fix_class_deadline_overrides_same_day_toggle.sql 롤백
-- fix_reserve_with_membership_operational_settings.sql / fix_reserve_class_block_after_start_draft_proposed.sql
-- 적용 직후 상태(당일예약 허용 검사가 booking_deadline_min과 무관하게 항상 실행되던 상태)로 되돌린다.
-- 두 함수의 본문 전체는 이 배치 이전 파일들과 동일하므로, 그 파일들을 다시 실행해도 같은 결과다.
-- 여기서는 "당일 예약 허용 여부" 블록의 조건만 원래대로 되돌리는 최소 변경으로 대체 제공한다.
