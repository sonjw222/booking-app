...
-- Track 2: 수업 시작 후 회원 셀프 예약 차단 (서버 측)
--
-- 조사 결과: reserve_class()에는 "예약 마감시간"(booking_deadline_min/calc_deadline('book'))
-- 체크만 있고, "수업이 이미 시작됐는지" 자체를 확인하는 코드가 전혀 없었다. 운영설정에 따라
-- (예: group_book_days_before=0, group_book_time="23:59") 예약 마감이 수업 시작 시각보다
-- 훨씬 늦게 잡히면, 이미 시작한 수업도 예약이 그대로 통과되는 상태였다 —
-- cancel_reservation()에는 이미 있는 "if now() >= v_class.start_time then raise exception..."
-- 와 대칭되는 체크가 reserve_class()에는 누락돼 있었던 것.
--
-- 수정: 승인된 센터 확인 직후, 예약 마감시간 계산 이전에 "수업 시작 이후면 무조건 차단"을
-- 추가한다. 나머지 로직(마감/오픈/당일예약/일일한도/대기한도/수강권 조건)은 전혀 바꾸지
-- 않는다 — CREATE OR REPLACE 안에서 이 한 블록만 새로 삽입.
-- ============================================================
...
begin
    ...
    -- [Track 2 신규] 수업이 이미 시작됐으면 운영설정/개별 수업 마감값과 무관하게 무조건 차단.
    -- cancel_reservation()의 "이미 시작되어 취소할 수 없어요" 체크와 대칭.
