  ...
  return (
    <div className="app-shell member-my-reservations">
      ...
      <SegmentedTabs value={filter} onChange={setFilter} label="예약 내역 종류"
        items={[{ value: "upcoming", label: "예정된 예약" },{ value: "past", label: "지난 예약" }]} />

      {loading ? <Loading /> : shown.length === 0 ? (
        <EmptyState icon="calendar" title={filter === "upcoming" ? "예정된 예약이 없어요" : "지난 예약이 없어요"}
                    ...
                    <div className="hist-main">
                      <div className="hist-title">
                        ...
                        {memberFacingBadge(h.reservationType as ReservationType) && (
                          <span className="profile-tag sm reservation-type-tag">{memberFacingBadge(h.reservationType as ReservationType)}</span>
                        )}
                      </div>
