...
const CATEGORIES = [
  { icon: "skate", label: "피겨스케이팅" },
  { icon: "pilates", label: "필라테스" },
  { icon: "ballet", label: "발레" },
  { icon: "rhythm", label: "리듬체조" },
  { icon: "yoga", label: "요가" },
  { icon: "boxing", label: "복싱" },
  { icon: "swim", label: "수영" },
  { icon: "golf", label: "골프" },
];

const CATEGORY_ICONS: Record<string, Parameters<typeof UiIcon>[0]["name"]> = {
...
export default function Home() {
  ...
  const visibleCategories = useMemo(
    () => (catList.length > 0 ? catList.map((c) => ({ icon: CATEGORY_ICONS[c.label] ?? "grid", label: c.label })) : CATEGORIES).slice(0, 8),
    [catList]
  );
  const visibleClasses = useMemo(() => {
    const source = myUpcoming.length > 0 ? myUpcoming : classes;
    const qaPattern = /(?:settings|audit|read[-_ ]?audit|^res[-_ ]?\d|test|테스트)/i;
    return source.filter((item) => item.title?.trim().length >= 2 && !qaPattern.test(item.title));
  }, [classes, myUpcoming]);
    ...
    <div>
      <div className="app-shell member-home">
        ...
        {validBanners.length > 0 ? (
          ...
          >
            <a className="hero" href={validBanners[bannerIdx]?.linkUrl || "/reservation"} style={{ display: "flex", textDecoration: "none", backgroundImage: centers[0]?.photoUrl ? `linear-gradient(180deg,rgba(0,0,0,.04),rgba(0,0,0,.74)),url(${centers[0].photoUrl})` : undefined }}>
          ...
          <a className="hero" href="/reservation" style={{ display: "flex", textDecoration: "none", backgroundImage: centers[0]?.photoUrl ? `linear-gradient(180deg,rgba(0,0,0,.04),rgba(0,0,0,.74)),url(${centers[0].photoUrl})` : undefined }}>
        ...
        <div className="cat-grid">
          {visibleCategories.map((cat) => (
            <a className="cat-item" key={cat.label} href={`/category/${encodeURIComponent(cat.label)}`}>
              <div className="cat-icon"><UiIcon name={cat.icon as Parameters<typeof UiIcon>[0]["name"]} size={27} /></div>
            ...
            {visibleClasses.slice(0, 3).map((c) => {
              ...
              return (
                <a className="home-class-row" key={c.id} href={`/center/${c.centerId}`}>
                  <div className={`home-class-photo ${center?.photoUrl ? "" : "photo-fallback"}`} style={{ backgroundImage: center?.photoUrl ? `url(${center.photoUrl})` : undefined }} aria-label={`${center?.name ?? c.centerName} 클래스 이미지`} />
