...
export default function ManagerChrome() {
  ...
  const showBack = !rootScreens.has(pathname);
  return (
    <header className="manager-chrome">
      <div className="manager-chrome-main">
        <div className="app-chrome-title">
          {showBack && <button className="app-back-btn" onClick={() => pathname === "/manager/settings" ? router.push("/manager") : router.back()} aria-label="뒤로가기">‹</button>}
          <h1>{title}</h1>
        </div>
        {pathname === "/manager" ? (
          <a className="manager-member-link" href="/" aria-label="회원 화면">
            <UiIcon name="user" size={20} /><span>회원 화면</span>
          </a>
        ) : (
          <a className="manager-home-link" href="/manager" aria-label="관리 홈">
            <UiIcon name="grid" size={20} /><span>관리 홈</span>
          </a>
        )}
      </div>
    </header>
